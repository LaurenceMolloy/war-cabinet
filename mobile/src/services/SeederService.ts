...
export type SeederScenario = {
  rank: string;
  description: string;
  apply: (db: any) => Promise<void>;
};

export const SEEDER_SCENARIOS: Record<string, SeederScenario> = {
  'foundation_basic_cabinets_and_categories': {
    rank: 'SERGEANT',
    description: 'Deploys 4 standard/freezer cabinets and 4 basic item types. Useful for fundamental navigation testing.',
    apply: async (db) => await SEEDER_SCENARIOS['foundation_basic_grid_with_types'].apply(db)
  },
  'foundation_basic_grid_with_types': {
    rank: 'SERGEANT',
    description: 'Identical to basic cabinets, establishing a clean 4x4 grid of categories and cabinets.',
    apply: async (db) => {
      ...
      try {
        ...
        console.log('[E2E] FOUNDATION SEED COMPLETE.');
      } catch (e: any) {
  ...
  'foundation_oversight': {
    rank: 'SERGEANT',
    description: 'Minimalist setup for logistics testing. 4 cabinets, 1 category, and 1 item type with baseline stock.',
    apply: async (db) => {
  ...
  'foundation_with_inventory': {
    rank: 'SERGEANT',
    description: 'Full 4-cabinet grid populated with 4 distinct item types and baseline inventory levels.',
    apply: async (db) => {
      ...
      try {
        ...
        console.log('[E2E] INVENTORY SEED COMPLETE.');
      } catch (e: any) {
        console.error('[E2E] SEED FAILURE:', e);
        if (Platform.OS === 'web') {
          window.localStorage.setItem('E2E_SEED_ERROR', e.message || 'Unknown DB Error');
  ...
  'foundation_heavy_inventory': {
    rank: 'VETERAN',
    description: 'DYNAMIC ENVIRONMENT: 6 cabinets, 12 categories, 60+ products. High-variance stock levels, brands, ranges, and photos. Semi-randomly generated for deep stress testing.',
    apply: async (db) => {
